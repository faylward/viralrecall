...
def run_program(input, project, database, window, phagesize, minscore, minvog, evalue, cpus, plotflag, redo, flanking, batch, summary_file, contiglevel):
	...
	if contiglevel:
		for rep in reps:
			...
			voghits = len([i for i in df3["vogbit"].tolist() if i > 0])
			...
			data = pandas.Series([replicon, minval, maxval, contig_length, score, num_prot, voghits, pfamhits, num_capsid, num_wedge, num_portal, num_term, num_tail, num_spike], name=project)
		...
		if batch:
			...
			if summary.shape[1] > 0:
				...
				summary.columns = ['replicon', 'start_coord', 'end_coord', 'contig_length', 'score', 'num_ORFs', 'num_voghits', 'num_pfamhits', 'capsid', 'wedge', 'portal', 'terminase', 'tail', 'spike']
...
def main(argv=None):
	...
	if batch:
		...
		for i in file_list:
			...
			run_program(newinput, newproject, database, window, phagesize, minscore, minvog, evalue, cpus, plotflag, redo, flanking, batch, summary_file, contiglevel)
	else:
		summary_file = 1
		run_program(input, project, database, window, phagesize, minscore, minvog, evalue, cpus, plotflag, redo, flanking, batch, summary_file, contiglevel)
